...
layerStats(pearson.bat_lim, 'pearson', na.rm=TRUE)
...
[12] Now we just have to obtain p-values for each of these correlation coefficients. I believe we will want to use the corLocal() function under the Raster package
```{r}
x <- battus_heat
y <- limenitis_heat

cor.local_bat_lim <- corLocal(x, y, test = TRUE ) # Not sure if we want to play w/ ngb (neighborhood size argument, like our moving window dimensions).
plot(x)


layerStats(cor.local_bat_lim, 'pearson', na.rm = TRUE)
```