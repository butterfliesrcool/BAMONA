...
>>>>>>> 55c26761681a6f94113588c4d4035fdfa0f3ffd1
...
length(diana$Record.Number)
diana
>>>>>>> eb390217d7dca49989444995093c54a32626c5d9
```
...
```
```{r}
#### do this at 90%, 80%, 70%, 60%, 50%, 40%, 30%, and 20%. might be clearer than a heat map?
######### also figure out how to analyze this statistically 
bat.lim.20 <- b + geom_sf(data = bat.kern20, color = "salmon1", fill = "salmon1", alpha = 0.3) + 
              geom_sf(data = lim.kern20, color = "royalblue1", fill = "royalblue1", alpha = 0.5) +
...
us
USA <- get_stamenmap(us, zoom = 5, maptype = "toner-lite") %>% ggmap() 
USA
