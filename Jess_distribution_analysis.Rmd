...
rcor_bat_lim_nor <- rasterCorrelation(battus_heat_north, limenitis_heat_north, s = 5, type = "pearson")
plot(rcor_bat_lim_nor)

rcor_bat_pol_nor <- rasterCorrelation(battus_heat_north, polyxenes_heat_north, s = 5, type = "pearson")
plot(rcor_bat_pol_nor)

rcor_bat_tro_nor <- rasterCorrelation(battus_heat_north, troilus_heat_north, s = 5, type = "pearson")
plot(rcor_bat_tro_nor)

rcor_bat_gla_nor <- rasterCorrelation(battus_heat_north, glaucus_heat_north, s = 5, type = "pearson")
plot(rcor_bat_gla_nor)

rcor_bat_dia_nor <- rasterCorrelation(battus_heat_north, diana_heat_north, s = 5, type = "pearson")
plot(rcor_bat_dia_nor)
```
...
[18] Applying the layerStats function to all rasters - North only
```{r}
...
[19] p-values for each (northern) pairwise comparison
```{r}
p_bat_lim_nor <- cor_p_values(battus_heat_north,limenitis_heat_north)
show(p_bat_lim_nor)
...
[20] Now the interesting part! Comparing Limenitis vs. the other mimics in the North...
```{r}
rcor_lim_bat_nor <- rasterCorrelation(limenitis_heat_north, battus_heat_north, s = 5, type = "pearson")
plot(rcor_lim_bat_nor) # same as before

rcor_lim_pol_nor <- rasterCorrelation(limenitis_heat_north, polyxenes_heat_north, s = 5, type = "pearson")
plot(rcor_lim_pol_nor)

rcor_lim_tro_nor <- rasterCorrelation(limenitis_heat_north, troilus_heat_north, s = 5, type = "pearson")
plot(rcor_lim_tro_nor)

rcor_lim_gla_nor <- rasterCorrelation(limenitis_heat_north, glaucus_heat_north, s = 5, type = "pearson")
plot(rcor_lim_gla_nor)

rcor_lim_dia_nor <- rasterCorrelation(limenitis_heat_north, diana_heat_north, s = 5, type = "pearson")
plot(rcor_lim_dia_nor)
```

[21] Pearson's CC for Limenitis vs. other
```{r}
pearson.lim_bat_nor <- stack(limenitis_heat_north, battus_heat_north)
layerStats(pearson.lim_bat_nor, 'pearson', na.rm=TRUE)
...
[22] Calculating p-values for the northern Limenitis vs. mimics comparison
```{r}
p_lim_bat_nor <- cor_p_values(limenitis_heat_north,battus_heat_north)
show(p_lim_bat_nor)

p_lim_pol_nor <- cor_p_values(limenitis_heat_north,polyxenes_heat_north)
show(p_lim_pol_nor)

p_lim_tro_nor <- cor_p_values(limenitis_heat_north,troilus_heat_north)
show(p_lim_tro_nor)

p_lim_gla_nor <- cor_p_values(limenitis_heat_north,glaucus_heat_north)
show(p_lim_gla_nor)

p_lim_dia_nor <- cor_p_values(limenitis_heat_north,diana_heat_north)
show(p_lim_dia_nor)
```

[23] Repeating this analysis for the SOUTHERN part of each species' range.
```{r}
...
[24] Pearson's correlation coefficient (rasters) between each southern heatmap (again Battus vs. each mimic)
```{r}
rcor_bat_lim_sou <- rasterCorrelation(battus_heat_south, limenitis_heat_south, s = 5, type = "pearson")
plot(rcor_bat_lim_sou)

rcor_bat_pol_sou <- rasterCorrelation(battus_heat_south, polyxenes_heat_south, s = 5, type = "pearson")
plot(rcor_bat_pol_sou)

rcor_bat_tro_sou <- rasterCorrelation(battus_heat_south, troilus_heat_south, s = 5, type = "pearson")
plot(rcor_bat_tro_sou)

rcor_bat_gla_sou <- rasterCorrelation(battus_heat_south, glaucus_heat_south, s = 5, type = "pearson")
plot(rcor_bat_gla_sou)

rcor_bat_dia_sou <- rasterCorrelation(battus_heat_south, diana_heat_south, s = 5, type = "pearson")
plot(rcor_bat_dia_sou)
```

[25] Finding Pearson's correlation coefficient for each mimetic species vs. Battus in the South only
```{r}
...
[26] Calculating p-values for each pairwise comparison of Battus vs. others in the South
```{r}
p_bat_lim_sou <- cor_p_values(battus_heat_south,limenitis_heat_south)
show(p_bat_lim_sou)
...
[27] Pearson's CC plots for Limenitis vs. other mimics in the South
```{r}
rcor_lim_bat_sou <- rasterCorrelation(limenitis_heat_south, battus_heat_south, s = 5, type = "pearson")
plot(rcor_lim_bat_sou)

rcor_lim_pol_sou <- rasterCorrelation(limenitis_heat_south, polyxenes_heat_south, s = 5, type = "pearson")
plot(rcor_lim_pol_sou)

rcor_lim_tro_sou <- rasterCorrelation(limenitis_heat_south, troilus_heat_south, s = 5, type = "pearson")
plot(rcor_lim_tro_sou)

rcor_lim_gla_sou <- rasterCorrelation(limenitis_heat_south, glaucus_heat_south, s = 5, type = "pearson")
plot(rcor_lim_gla_sou)

rcor_lim_dia_sou <- rasterCorrelation(limenitis_heat_south, diana_heat_south, s = 5, type = "pearson")
plot(rcor_lim_dia_sou)
```
[28] Pearson's CC for Limenitis vs. others, Southern ranges
```{r}
pearson.lim_bat_sou <- stack(limenitis_heat_south, battus_heat_south)
layerStats(pearson.lim_bat_sou, 'pearson', na.rm=TRUE)
...
[29] p-values for Limenitis vs. others in the South
```{r}
p_lim_bat_sou <- cor_p_values(limenitis_heat_south,battus_heat_south)
show(p_lim_bat_sou)

p_lim_pol_sou <- cor_p_values(limenitis_heat_south,polyxenes_heat_south)
show(p_lim_pol_sou)

p_lim_tro_sou <- cor_p_values(limenitis_heat_south,troilus_heat_south)
show(p_lim_tro_sou)

p_lim_gla_sou <- cor_p_values(limenitis_heat_south,glaucus_heat_south)
show(p_lim_gla_sou)

p_lim_dia_sou <- cor_p_values(limenitis_heat_south,diana_heat_south)
show(p_lim_dia_sou)
```
