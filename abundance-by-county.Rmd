...
Data Summary:
```{r}
head(bat) ## Battus sighting counts by county
head(lim) ##Limenitis sighting counts by county 
head(pol) ## Polyxenes sighting counts by county 
head(gla) ## Glaucus sighting counts by county 
head(dia) ## Diana sighting counts by county 
head(tro) ## Troilus sighting counts by county 

head(abundance_and_area) ## Sighting counts and land area (km^2) & summed species density for each county 
head(absolute_abundance) ## Sighting counts divided by land area: standardized sightings/ km^2
head(relative_abundance) ## proportion (percent) of sightings in each county that represent each species & species richness and species density in each county
```
